...
- `npx tj` runs the [`tj` npm wrapper](https://www.npmjs.com/package/tj) — a thin
  ...
  so `npx tj quickstart --since 7d`, `npx tj optimize`, etc. all work.
...
**Requirements:** a Python runner — `uv` (recommended) or `pipx`. `npx tj` prints
