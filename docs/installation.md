...
To confirm telemetry is actually flowing (not just that setup completed), use `tj onboard --verify` (polls for the first span right after setup) or run `tj ping` any time afterward (emits one labeled test span and reports where it landed). Both are covered in the [CLI reference](cli-reference.md). Running onboarding unattended (CI, Docker, a script)? See [Non-interactive / CI setup](ci-setup.md).
