...
| **Analyzers (thirteen: downsize / cache / cache-recommend / resend / script / trim / reuse / subagent / summarize / verbosity / relearn / budget-projection / deadweight)** | Yes: all thirteen | Partial, eleven of thirteen: `deadweight` reads Claude Code transcripts on disk, and `relearn`'s transcript lane is Claude Code only while its span lane deliberately skips coding agents so the same failure is never counted twice | Partial: no transcripts on disk, so no `deadweight`; no agent instruction files to scan, so no `summarize`; and `sub_agent_id` is never populated, so no `subagent`. `relearn` runs its span lane, which detects and advises but never applies (there is no workspace to write a fix into) | Partial, same as the SDK column |
