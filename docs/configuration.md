...
### The ingest secret is global, not per project

Discovery picks one file, but `security.ingest_secret` is machine-wide: the daemon is launched from a
unit file naming the global config explicitly, so it always authenticates with the secret stored
there, whatever directory anything else runs from. A different secret in a project-local
`.tj/config.toml` is one the daemon can only reject, and spans signed with it are discarded with a
401 that nothing surfaces.

So `tj onboard` never mints a second secret: it adopts the global one, and rewrites a project-local
config that carries a stale value. The global value always wins, because the managed shell block and
every already-configured integration carry it. `tj doctor` reports any divergence it still finds,
alongside a live probe of the endpoint itself.

### Telemetry endpoint host

`TJ_OTEL_HOST` overrides the host `tj onboard` writes into the OTLP endpoint in your shell profile.
The default is loopback, and the Docker gateway name is used only when `tj` is itself running inside
a container. Set this when neither describes your setup, for example a shell profile exported into a
container, or a daemon on another machine.
...
[security]
ingest_secret = "..."   # generated by tj onboard, in the GLOBAL config only
