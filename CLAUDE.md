...
- **output_cap / `tj hook cap-output`** and the **`verbosity`** optimize analyzer were removed — both measured negative (output_cap: +5.6% whole-session cost on Claude Code, since CC pre-truncates Bash output to ~30 KB before the hook ever sees it; see `.specs/2026-07-02-tj-output-trim-hook-{design,AB-result}.md`). `tj onboard`/`tj uninstall` still strip an already-installed legacy cap-output hook entry from `~/.claude/settings.json` for prior-release users.
...
**The MCP is an SDK / API surface, not a Claude Code / Codex one.** It puts tj *in the request path* — the right place for SDK / API integrations doing real-time enforcement/policy/budgets. It is deliberately **not** wired for Claude Code / Codex subscription users: an in-loop MCP is a per-turn token burden on them (a measured A/B showed **+36%** model-weighted tokens vs a no-tj control). Those users get tj **out-of-band**: the zero-token statusline (`tj statusline`, wired by `tj onboard --claude-code`) plus OTel telemetry ingest. `tj mcp` still works for anyone who invokes it; onboarding just no longer defaults CC/Codex users into it.
