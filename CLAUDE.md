...
13. **PyPI package name is `tokenjam`, not `ocw`** — `pip install tokenjam` is the correct install command. The CLI command is `tj` (also `jm`) and the Python package directory is `tj/`. The published package name on PyPI is `tokenjam`. Never write `pip install ocw` in docs, examples, or comments.
