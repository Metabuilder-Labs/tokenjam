...
- **`tj quickstart`** (`cmd_quickstart.py`) — the **zero-install / zero-config first run** (issue #6). Opens a **transient `InMemoryBackend`** (nothing written to `~/.tj`, no config read/written, no daemon started or contacted), backfills `~/.claude/projects/*.jsonl` into it via `ingest_claude_code`, then renders quota composition (reusing `core/context_diagnostic.py` from #4) + a session timeline (`core/session_timeline.py`). Output leads with ccusage-parity framing ("reads the same files ccusage does"). It is in `no_db_commands` so the CLI never opens the on-disk DB / trips the daemon lock for it. The intended zero-install invocations are `npx tj quickstart` (via the `npm-wrapper/` package) and `uvx --from tokenjam tj quickstart`. `tj onboard` remains the opt-in "go deeper" (daemon/MCP/live) path. **Bare `tj` (no subcommand) prints the branded home screen** (`cli/home.py` `print_home()`, #240) — banner + next-best-action, reading only config presence (never the DB) — not quickstart; `--help`/`--version` are eager and still short-circuit.
...
- **Testing the UI (no JS runner in CI):** the Python `test` job can't run JS, so UI fixes are guarded by **static-grep regression tests** in `tests/unit/test_lens_ui_regression.py` (assert buggy patterns are *gone* and new helpers are present) plus `test_ui_offline.py`. When iterating locally, validate syntax with `node --check` on the extracted `<script type="module">` block, and verify visually by running `tj serve` (or a seeded `create_app` + uvicorn on an alt port) and screenshotting with headless Chrome — there is intentionally no Playwright/Cypress. **Seed a LONG dense session for any layout-bearing change** (dozens of subagents, hours of spans with idle gaps, temp-file reads mixed in): label collisions, path truncation and noise-swamping only appear at density, and a short seeded mock + string-pin tests pass right through them (meta ticket #56 shipped an undecodable Map board exactly this way).
