...
- **`tj quickstart`** (`cmd_quickstart.py`) — the **zero-install / zero-config first run** (issue #6). Opens a **transient `InMemoryBackend`** (nothing written to `~/.tj`, no config read/written, no daemon started or contacted), backfills `~/.claude/projects/*.jsonl` into it via `ingest_claude_code`, then renders quota composition (reusing `core/context_diagnostic.py` from #4) + a session timeline (`core/session_timeline.py`). Output leads with ccusage-parity framing ("reads the same files ccusage does"). It is in `no_db_commands` so the CLI never opens the on-disk DB / trips the daemon lock for it. The intended zero-install invocations are `npx tokenjam quickstart` (via the `npm-wrapper/` package) and `uvx --from tokenjam tj quickstart`. `tj onboard` remains the opt-in "go deeper" (daemon/MCP/live) path. **Bare `tj` (no subcommand) prints the branded home screen** (`cli/home.py` `print_home()`, #240) — banner + next-best-action, reading only config presence (never the DB) — not quickstart; `--help`/`--version` are eager and still short-circuit.
