...
- **output_cap / `tj hook cap-output`** (with `tokenjam/core/output_cap.py`, `tokenjam/cli/cmd_hook.py`, and the `tj savings` CLI) was removed — measured negative (+5.6% whole-session cost on Claude Code, since CC pre-truncates Bash output to ~30 KB before the hook ever sees it; see `.specs/2026-07-02-tj-output-trim-hook-{design,AB-result}.md`). `tj onboard`/`tj uninstall` still strip an already-installed legacy cap-output hook entry from `~/.claude/settings.json` for prior-release users. `core/savings_log.py` survives as a lock-free JSONL-sink utility (`hooks_dir`) that `core/recommendations.py` reuses.
...
- **`tj optimize`** (`cmd_optimize.py`) — twelve analyzers, registry-driven. **Analyzers are positional args** (not `--finding <name>`): `tj optimize downsize cache trim` runs three; bare `tj optimize` runs all. Registered names: `downsize`, `cache`, `cache-recommend`, `script`, `reuse`, `trim`, `subagent`, `summarize`, `verbosity`, `deadweight`, `relearn`, `budget-projection` (the live list is `ANALYZER_REGISTRY`; derive it from there rather than trusting this line). Flags: `--since 30d`, `--budget <provider>`, `--budget-usd <amount>`, `--compare <period>` (window-cost diff vs prior period; accepts `previous` / `last-week` / `last-month` / `last-7d` / `last-30d` / `YYYY-MM-DD:YYYY-MM-DD`), `--export-config <target>` (writes a routing snippet — currently `claude-code` — under `~/.config/tokenjam/exports/`; no `--apply` flag by design). Plan-tier-aware rendering: subscription users see "implied API value" framing and token-share savings (never dollar "spend"); local users see token-only framing; unknown-plan users see dollar figures suppressed with a `tj onboard --reconfigure` hint. Works alongside a running `tj serve` via the `/api/v1/optimize` HTTP fallback when the DuckDB write lock is held by the daemon.
...
- **Backfill adapters** — `docs/backfill/overview.md` lists the five sources (`claude-code` / `codex` / `langfuse` / `helicone` / `otlp`) with the partnership-posture framing; per-adapter pages document modes (URL / file / on-disk root), field mapping, idempotency, and v1 limitations.
