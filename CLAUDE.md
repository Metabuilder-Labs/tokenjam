...
- **`tj quickstart`** (`cmd_quickstart.py`) — the **zero-install / zero-config first run** (issue #6), and what bare `tj` (no subcommand) routes to. Opens a **transient `InMemoryBackend`** (nothing written to `~/.tj`, no config read/written, no daemon started or contacted), backfills `~/.claude/projects/*.jsonl` into it via `ingest_claude_code`, then renders quota composition (reusing `core/context_diagnostic.py` from #4) + a session timeline (`core/session_timeline.py`). Output leads with ccusage-parity framing ("reads the same files ccusage does"). It is in `no_db_commands` so the CLI never opens the on-disk DB / trips the daemon lock for it. The intended invocations are `npx tokenjam` (via the `npm-wrapper/` package; the bare `tj` name is squatted on npm) and `uvx --from tokenjam tj`. `tj onboard` remains the opt-in "go deeper" (daemon/MCP/live) path. **The root group is `invoke_without_command=True`**: bare `tj` invokes quickstart; `--help`/`--version` are eager and still short-circuit.
