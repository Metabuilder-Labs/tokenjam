...
47. **`Path.expanduser()` reads the `HOME` env var and never calls `pathlib.Path.home()`, so a test that patches `Path.home` does not redirect it.** Product code resolving `~/...` keeps reading the session-wide fake HOME from `conftest.py`'s autouse `_tj_isolated_home` fixture, so such a test passes only on state an earlier test left there and fails when run alone. Redirect with `monkeypatch.setenv("HOME", ...)`; the suite runs under `-n auto`, where cross-test leakage is a failure rather than a hidden crutch.

48. **A control that only sets client state is a control that does nothing — trace the selection to the request before believing the surface it drives.** A "Viewing as" picker whose value never leaves the browser leaves every affordance downstream keyed to whatever the SERVER happened to compute, so the page renders one persona's answer under another persona's label and looks entirely plausible. The tell is a payload field that names a persona (`persona`, `persona_disabled_analyzers`) while the request that fetched it carried none.
   (a) **Where the answer cannot be recomputed on demand, compute the UNION once and slice on read — but ONLY a SET can be sliced, never a POPULATION.** No request path here may run an analyzer, so a per-selection recompute is not available; the background pass therefore gates on the INTERSECTION of every selectable persona's disabled set and each read narrows to the one asked for. Slice SERVER-side, so a surface that forgets the filter cannot render what it should not; and keep the artifact's own recorded identity (which persona the corpus IS) separate from the view being served, or every consumer of the stored artifact starts disagreeing about which one it holds. **The limit is the whole lesson and this rule originally missed it:** a set of analyzer NAMES is separable, so union-then-slice is sound for it; the ROWS those analyzers aggregate are not, because one sum over two populations contains both and no read-time filter can extract either. Applying the set gate alone leaves every surviving analyzer measuring the whole corpus and publishing it under whichever label the reader picked — which is the same defect this rule is about, one layer down, and it survived the first fix because the gate looked like the whole feature. So a per-selection figure is computed under that selection's scope (one pass each, stored separately) or it is not computed at all; and a gate INSIDE such a pass reads the pass's scope, never the corpus's own classification, which disables nothing when the corpus is mixed.
   (b) **Widening what a shared artifact contains is a change to every blind reader of it.** Anything that walked "every finding carrying a dollar field" was implicitly relying on the gate having already removed the rest. Grep those walkers and scope them explicitly before widening.
   (c) **Two figures rendered side by side must be scoped alike, even when they come from different endpoints.** Scoping the analyzer list but not the "biggest single cause" line above it named, as the top opportunity, an analyzer the list had just correctly dropped.

49. **Preact swallows a re-render exception into its rerender queue: the page silently stops updating, the console stays EMPTY, and the symptom is a permanent skeleton that reads as a slow fetch.** So the UI's most expensive failure mode produces no error anywhere, and a static-grep test suite cannot see it — deleting a binding while a reference survives is invisible until the branch holding that reference is reached, which may need data the dev machine does not have (an ACTIVE agent card, a populated list). Grep every reference before deleting a `const`, and pin removed identifiers by name so a re-introduction fails in CI. To diagnose one: hook Preact's error handler from the page — `import('/ui/vendor/preact.js').then(m => { m.options.__e = m.options._catchError = cap })` — which reports the exception the tree never surfaced. Confirm the freeze is the TREE and not the environment first (a `setInterval` still ticking proves JS is healthy), and confirm against the committed build before assuming your own change caused it. Corollary: on a large corpus `/status` can take ~10s, so "still a skeleton" is ambiguous — wait for a request count to advance before calling it frozen.

50. **A test suite that only MATCHES STRINGS against `ui/index.html` stays green over a page that does not render at all — pin that the module PARSES.** A syntax error anywhere in the dashboard's module script is total: the browser executes none of it, `#app` stays empty, every page is blank, and the console is empty because nothing ran to log anything. A file in that state still contains every string every UI test greps for, so the whole UI suite passes. This is not hypothetical — a comment written in the `${ /* … */ '' }` form that is correct BETWEEN markup nodes was placed inside a ternary EXPRESSION, where it is a syntax error, and nothing in the suite noticed. `tests/unit/test_lens_module_parses.py` runs the extracted module through `node --check`; it is the cheapest test in the suite and the only one that can fail for the most catastrophic reason. Two corollaries: a `/* … */` comment cannot contain `*/`, so prose about the interpolating form has to describe it rather than quote it; and string-matching tests are fine for CONTENT but can never establish that the page works — dogfood the rendered UI before believing them.
