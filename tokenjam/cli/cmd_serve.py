...
def cmd_serve(ctx: click.Context, host: str | None, port: int | None,
    ...
    # Self-improve loop: keep the pothole-detector cache warm on a schedule so
    ...
    scheduler.add_job(_pothole_job, "interval", hours=6)
    ...
    async def _lifespan(_app: FastAPI) -> AsyncIterator[None]:
        ...
        if proxy_runner is not None:
            proxy_runner.start()
        # Kick the pothole detector's first pass now (own connection, own
