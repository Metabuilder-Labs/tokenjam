...
- **`cloud_sync.py`** — the bridge to TokenJam Cloud, sender side (ledger W5). `run_sync(db, config)` is one pass over three streams read straight off `db.conn` with literal SQL (sessions first, then `session_commits`, then spans, so the receiver never sees a row naming a session it lacks): sessions and commits go to `POST /api/v1/ledger/sessions` as `{"sessions": [...], "session_commits": [...]}` (contracts §6), spans to the existing `POST /api/v1/spans` re-encoded as OTLP JSON through the same attribute names `otel/otlp_parsing.parse_otlp_span` reads (Cloud runs that parser, so the round trip is the test). Each stream resumes from an arrival-order high-water mark in `~/.tj/cloud_sync.json` (`spans.ingested_at`, `sessions.updated_at`, `session_commits.matched_at`; migration 25 added the first two, and every session writer in `db.py` and `framing.py` stamps `updated_at = now()`, a new writer must too); `start_sync` is the daemon-thread wrapper with the Critical Rule 45 fatal classification. Every session row and spans resource carries `~/.tj/install_id` (`ensure_install_id`; Cloud counts connected developers off it), and a pass that could neither read nor write it says so on `SyncReport.install_id_missing`, which `tj init --cloud` prints. The CLI's initial push (`cli/ledger_cloud.py::_prepare_then_push`) runs `transcript_sync.refill_session_context`, then `shipped.match_sessions_to_commits`, then `run_sync`, in that order, so what leaves is what the ledger can join on (issue #770: the first real push carried zero commits because every local session still had NULL repo context). `content_gate` is the only place the §6 content rule lives: everything stripped unless `[cloud] forward_content` is on, in which case the local `[capture]` toggles decide, through the same `strip_captured_content` the ingest uses plus a leaf-name sweep. Never imports the CLI; `cli/ledger_cloud.py` renders.

- **`backfill.py`** — parses Claude Code on-disk session JSONL into `NormalizedSpan`s, recomputing cost from `pricing/models.toml` because the on-disk format has no `cost_usd`. Tolerates the dated `claude-<family>-<ver>-YYYYMMDD` model-name suffixes Anthropic ships, via `core/pricing.py.get_rates()`, which strips the trailing 8-digit date suffix when no exact match exists. Idempotency relies on deterministic span IDs derived from `(session_id, message uuid)` / `(session_id, tool_use id)`. **Plan tier:** `ingest_claude_code(db, …, config=…)` resolves `plan_tier` from `config.budgets["anthropic"].plan` (Claude Code is always Anthropic) and stamps it on each `SessionRecord`, mirroring the live `IngestPipeline._resolve_plan_tier` so backfilled sessions aren't all `"unknown"` (#176) — pass `config` from callers (`cmd_backfill`, `tj onboard`). The Langfuse/Helicone/OTLP adapters create **no** `SessionRecord` (spans only), so there is no plan tier to propagate there. `ingest_claude_code` raises `BackfillUnavailable` for a backend with neither `conn` nor `insert_span` (the serve-mode shim) rather than failing one span at a time; `tj backfill claude-code` under a running daemon is handed to the daemon through the data-access seam (`cli/data_access.py::claude_code_backfill` → `POST /api/v1/backfill/claude-code`) before ever reaching it. The transcript context REFILL (`transcript_sync.refill_session_context`) is the sibling that heals sessions ingested without repo context; see `ingest-accounting.md`.
...
`ApiBackend` — the HTTP shim the CLI uses when `tj serve` holds the DB write-lock. It is read-only
with one deliberate write, `upsert_session` (`POST /api/v1/sessions/upsert`, the daemon's own
`upsert_session`; it exists for the transcript context refill and is parity-covered by a
write-then-read spec). The shim reconstructs objects from JSON, so it diverges from the DB backend
*silently* (dropped `cache_write_tokens` on daemon-fetched spans, cache columns zeroed,
`get_daily_cost` returning a cumulative total, a `get_session` with no repo context). That whole
class of bug is guarded by `tests/integration/test_storage_backend_parity.py`, which runs each
faithfully-mirrored method against both the DuckDB backend and the `ApiBackend` shim — over a
**real in-process uvicorn server**, not the ASGI shortcut a sync `ApiBackend` can't use — and fails
on any divergence.
