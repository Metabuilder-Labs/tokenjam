...
- **Three routes exist so a CLI that found the daemon holding the DuckDB lock can still do what needs the connection** (issue #770; all `require_api_key`-gated, none runs git or a bulk write on the request thread). `POST /sessions/upsert` (`sessions.py`) is one `DuckDBBackend.upsert_session` carried over HTTP for `ApiBackend.upsert_session` (the transcript context refill's one write; body `{"session": SessionRecord.to_dict(), "accumulate_totals": bool}`), and `POST /sessions/ingested-ids` takes `missing_context: true` to answer the refill's candidate set through the shared `SESSION_CONTEXT_MISSING_SQL`. `POST /backfill/claude-code` (`backfill.py`) starts the daemon's own transcript catch-up (`transcript_sync.start_catch_up`) on its own thread and backend and returns `{started, running}`; one on-demand pass at a time. `POST /shipped/match?wait_s=` (`shipped.py`) runs `core/shipped.start_match` the same way and waits up to `wait_s` for it, answering `{started, running, completed, ...last result}`; a request arriving mid-pass joins it.
