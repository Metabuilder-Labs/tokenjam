...
# --------------------------------------------------------------------------- #
# Context audit page ("CleanMyMac for Claude Code")
# --------------------------------------------------------------------------- #

def test_context_audit_nav_entry_sits_after_review_inbox_and_before_faq(html):
    nav_lines = [
        ln for ln in html.splitlines()
        if 'class="nav-link' in ln and 'data-lens="improve"' in ln
    ]
    views = [re.search(r'data-view="([^"]+)"', ln).group(1) for ln in nav_lines]
    assert "context-audit" in views
    assert views.index("context-audit") > views.index("review")
    # FAQ must remain last (test_faq_nav_entry_is_last_in_improve_and_absent_from_observe
    # already pins this independently; this just confirms the new entry didn't
    # get appended after it).
    assert views.index("context-audit") < views.index("faq")
    assert "nav-child" not in [ln for ln in nav_lines if 'data-view="context-audit"' in ln][0]


def test_context_audit_registered_in_primary_views_and_lens_map(html):
    views_block = html[html.index("const PRIMARY_VIEWS = ["):html.index("const PRIMARY_KEYS")]
    assert "['context-audit', ContextAuditView]" in views_block.replace("  ", " ").replace("   ", " ") \
        or re.search(r"\[\s*'context-audit'\s*,\s*ContextAuditView\s*\]", views_block)

    lens_block = html[html.index("const VIEW_LENS = {"):html.index("const VIEW_LENS = {") + 800]
    lens_block = lens_block[:lens_block.index("};") + 2]
    assert "'context-audit': 'improve'" in lens_block


def test_context_audit_view_gates_totals_on_its_own_fetch(html):
    """Empty-state honesty: the page must not render ANY total before its one
    fetch has resolved (skeleton instead), and a failed refresh must not wipe
    already-loaded content (scoped banner instead) — same convention Budget
    and the Review inbox use."""
    start = html.index("function ContextAuditView")
    end = html.index("function ", start + 10)
    view = _no_comments(html[start:end])

    assert "if (!data) return html`" in view
    assert "TableRowsSkeleton(" in view
    # The blank-page branch only fires when there is no prior data at all.
    assert "if (error && !data) return html`" in view
    # A failed rescan renders a scoped banner, not a page wipe, once data exists.
    assert "band-msg err" in view


def test_context_audit_headline_sums_global_with_ONE_project_never_across_projects(html):
    """The headline legitimately sums the global floor with the CURRENTLY
    SELECTED project's floor (a session in that project really does carry
    both) — but must never sum two different projects' totals together,
    since no single session ever pays two projects' floors at once
    (mixed-basis guard, root anti-pattern 22)."""
    start = html.index("function ContextAuditView")
    end = html.index("function ", start + 10)
    view = _no_comments(html[start:end])

    assert "globalTokens + projectTokens" in view
    # `projectTokens` comes from ONE project (`currentProject`), never a
    # reduce/sum over the whole `projects` array.
    assert "const projectTokens = currentProject" in view
    assert "projects.reduce((s" not in view and "projects.reduce((sum" not in view


def test_context_audit_project_picker_shows_all_but_renders_one_scope(html):
    """26 project sections stacked vertically was the reported bug — only the
    picker (name + tokens, one line each) may reference every project; the
    actual per-project Class1/2/3/unloaded section must mount for exactly the
    selected one."""
    start = html.index("function ContextAuditView")
    end = html.index("function ", start + 10)
    view = _no_comments(html[start:end])

    # Exactly one <${ContextAuditScope} ...> for the project half (the other
    # is the always-shown global section).
    assert view.count("<${ContextAuditScope}") == 2
    assert "data.projects.map" not in view or "ContextAuditScope" not in html[
        html.index("data.projects.map", start):html.index("data.projects.map", start) + 400
    ]


def test_context_audit_class1_always_visible_class2_3_collapsed_by_default(html):
    """Class 1 renders directly (no <details>, never collapsed); Class 2 and
    Class 3 are wrapped in a <details> so they default to collapsed, per the
    progressive-disclosure redesign."""
    c1_start = html.index("function ContextAuditClass1Table")
    c1_end = html.index("function ", c1_start + 10)
    class1_src = _no_comments(html[c1_start:c1_end])
    assert "<details" not in class1_src

    details_start = html.index("function ContextAuditClassDetails")
    details_end = html.index("function ", details_start + 10)
    details_src = _no_comments(html[details_start:details_end])
    assert "<details" in details_src


def test_context_audit_class3_total_labelled_not_resident_and_deemphasised(html):
    """Class 3's aggregate token figure must never read as a bigger cost than
    Class 1's — it is explicitly labelled NOT resident and visually dimmed."""
    start = html.index("function ContextAuditClassDetails")
    end = html.index("function ", start + 10)
    src = _no_comments(html[start:end])
    assert "NOT resident" in src
    assert "classKey === 'class3'" in src
    assert "deemphasise" in src


def test_context_audit_unloaded_collapses_to_one_reassurance_line(html):
    start = html.index("function ContextAuditUnloaded")
    end = html.index("function ", start + 10)
    src = _no_comments(html[start:end])
    assert "<details" in src
    assert "not loaded, 0 tokens" in src


def test_context_audit_rows_sorted_by_tokens_descending_server_side(html):
    """The UI trusts the server's sort (`rows_for_display` sorts by tokens
    desc) rather than re-sorting client-side — pin that no client-side
    `.sort` reorders a class table's rows by anything else."""
    start = html.index("function ContextAuditClass1Table")
    end = html.index("function ContextAuditView")
    src = _no_comments(html[start:end])
    assert "rows.sort(" not in src
    assert ".sort((a, b) => a.tokens" not in src
