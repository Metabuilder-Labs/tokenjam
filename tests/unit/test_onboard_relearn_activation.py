"""Onboarding tail: relearn surfacing + first-fix-enable CTA (#179).

Exercises ``_run_relearn_first_fix`` directly (mirrors the direct-call +
...
themselves are already covered by ``test_relearn_apply.py``; this file only
...
def _no_op_apply(monkeypatch):
    ...
    def _boom(*a, **k):
        raise AssertionError("apply_relearn_fix should not have been called")
...
def test_thin_history_shows_watching_note_and_never_applies(monkeypatch, capsys, _no_op_apply):
    ...
    monkeypatch.setattr(relearn_mod, "compute_relearn_finding", lambda **k: _finding([]))
    cmd_onboard._run_relearn_first_fix(object(), port=7391, want_daemon=True)
...
def test_no_hook_quality_candidate_routes_to_review_later(monkeypatch, capsys, _no_op_apply):
    ...
    monkeypatch.setattr(
        relearn_mod, "compute_relearn_finding",
...
def test_non_interactive_shows_cta_but_never_prompts(monkeypatch, capsys, _no_op_apply):
    ...
    monkeypatch.setattr(relearn_mod, "compute_relearn_finding", lambda **k: _finding([_cluster()]))
...
def test_interactive_confirm_applies_and_enables_with_explicit_confirm(
    ...
    cluster = _cluster()
    monkeypatch.setattr(relearn_mod, "compute_relearn_finding", lambda **k: _finding([cluster]))
    monkeypatch.setattr(cmd_onboard, "_is_interactive", lambda: True)
    ...
    monkeypatch.setattr(relearn_apply, "apply_relearn_fix", _fake_apply)
    monkeypatch.setattr(relearn_apply, "enable_enforcement", _fake_enable)
...
def test_apply_refused_never_silently_forces(monkeypatch, capsys):
    ...
    def _refuse(*a, **k):
        raise relearn_apply.RelearnApplyRefused("an active session was seen")

    monkeypatch.setattr(relearn_apply, "apply_relearn_fix", _refuse)
    called_enable = []
    monkeypatch.setattr(
        relearn_apply, "enable_enforcement",
...
def test_no_daemon_points_at_tj_serve_first(monkeypatch, capsys, _no_op_apply):
    ...
    monkeypatch.setattr(relearn_mod, "compute_relearn_finding", lambda **k: _finding([low_confidence]))
    monkeypatch.setattr(cmd_onboard, "_is_interactive", lambda: True)
    cmd_onboard._run_relearn_first_fix(object(), port=7391, want_daemon=False)
    out = capsys.readouterr().out
